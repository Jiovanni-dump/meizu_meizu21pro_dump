...
let RamSizeGB="( $MemTotal / 1024 /1024 + 3)/4 * 4" #8G/12G/16G
customizeExtraFreeKbytes=$(getprop persist.sys.meizu.wmark_extra_free_kbytes_${RamSizeGB}g)

if [ -n "$customizeExtraFreeKbytes" ]
then
    if [ $customizeExtraFreeKbytes -lt $extra_free_kbytes ]
    then
        extra_free_kbytes=$customizeExtraFreeKbytes
        setprop sys.sysctl.extra_free_kbytes $customizeExtraFreeKbytes
    fi
fi
#}bsp@meizu.com
